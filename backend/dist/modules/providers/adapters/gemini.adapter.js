...
class GeminiAdapter {
    ...
    name = 'Google Gemini';
    type = 'cloud';
    get apiKey() { return config_1.config.geminiApiKey; }
    ...
    async chat(messages, model, options) {
        ...
        const res = await fetch(`${this.baseUrl}/models/${modelId}:generateContent?key=${this.apiKey}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
        if (!res.ok)
     ...
     */
    async analyzeImages(imagePaths, prompt, systemPrompt, model, options) {
        const modelId = model || 'gemini-2.5-flash';
     ...
     */
    async detectInImage(imagePath, query, model) {
