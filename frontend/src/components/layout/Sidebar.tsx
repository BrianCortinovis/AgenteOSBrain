...
const navItems = [
  { id: 'mappa' as const, label: 'Mappa', icon: 'M4 4h16v16H4z M4 12h16 M12 4v16' },
  ...
  { id: 'impostazioni' as const, label: 'Impostazioni', icon: 'M12.22 2h-.44a2 2 0 00-2 2v.18a2 2 0 01-1 1.73l-.43.25a2 2 0 01-2 0l-.15-.08a2 2 0 00-2.73.73l-.22.38a2 2 0 00.73 2.73l.15.1a2 2 0 011 1.72v.51a2 2 0 01-1 1.74l-.15.09a2 2 0 00-.73 2.73l.22.38a2 2 0 002.73.73l.15-.08a2 2 0 012 0l.43.25a2 2 0 011 1.73V20a2 2 0 002 2h.44a2 2 0 002-2v-.18a2 2 0 011-1.73l.43-.25a2 2 0 012 0l.15.08a2 2 0 002.73-.73l.22-.39a2 2 0 00-.73-2.73l-.15-.08a2 2 0 01-1-1.74v-.5a2 2 0 011-1.74l.15-.09a2 2 0 00.73-2.73l-.22-.38a2 2 0 00-2.73-.73l-.15.08a2 2 0 01-2 0l-.43-.25a2 2 0 01-1-1.73V4a2 2 0 00-2-2z M12 8a4 4 0 100 8 4 4 0 000-8z' },
...
export default function Sidebar() {
  ...
  return (
    <div style={{
        ...
        {navItems.map(item => (
          <button
            ...
            style={{
              ...
              background: currentView === item.id ? 'var(--bg-hover)' : 'transparent',
              color: currentView === item.id ? 'var(--accent-blue)' : 'var(--text-muted)',
              transition: 'all var(--transition)',
              border: currentView === item.id ? '1px solid var(--border-secondary)' : '1px solid transparent',
              flexShrink: 0,
            }}
          >
            <svg width="26" height="26" viewBox="0 0 24 24" fill="none"
              stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"
              style={{ opacity: currentView === item.id ? 1 : 0.45 }}
            >
              <path d={item.icon}/>
            </svg>
            <span style={{
              fontSize: 8, fontWeight: currentView === item.id ? 600 : 400,
              letterSpacing: 0.2, lineHeight: 1, opacity: currentView === item.id ? 1 : 0.5,
            }}>
              {item.label}
            </span>
          </button>
      ...
      <div style={{ padding: '6px 0', borderTop: '1px solid var(--border-primary)', width: '100%', textAlign: 'center' }}>
        <span style={{ fontSize: 7, color: 'var(--text-muted)' }}>v0.2</span>
      </div>
    </div>
  );
}
